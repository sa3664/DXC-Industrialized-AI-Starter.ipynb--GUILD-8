...
data0 = ai.read_data_frame_from_remote_csv(
    csv_url, col_names=[], delim_whitespace=False, header='infer')
data0.head()
...
reviews_per_month_list = data0[data0['reviews_per_month'].isnull(
)].index.tolist()
...
data0.dropna(how='any', subset=['last_review',
...
# clean the data
impute = False
data1 = ai.clean_dataframe(data0, impute, text_fields,
                           date_fields, numeric_fields, categorical_fields)
...
data1 = data1.drop(['host_id', 'id', 'last_review',
                    'longitude', 'latitude', 'number_of_reviews'], axis=1)
...
def datastory(api_endpoint, input, header):
    ...
    try:
        ...
        response = requests.post(
            api_endpoint, headers=headers, params=params, data=data)
...
data_layer = {
    "connection_string": "mongodb+srv://.d91jv.azure.mongodb.net/Guild8Airbnb?retryWrites=true&w=majority",
...
api_url = ai.publish_microservice(
    microservice_design, trained_model, verbose=False)
